...
while True:
    ...
    img_smoothed = cv2.GaussianBlur(img, (15, 15), 0)

    # Convert smoothed image to grayscale for plate detection
                ...
                if contour_area > min_area:
                    ...
                    if 1.5 <= aspect_ratio <= 5.0:
                        ...
                        if extracted_text is not None:
                            ...
                            # Save only the ROI part of the original image with a unique filename based on timestamp
                            original_roi_path = os.path.join(save_dir, f"original_roi_{int(time.time())}_{count}.jpg")
                            cv2.imwrite(original_roi_path, plate_roi)
                            print("Original ROI Image Saved:", original_roi_path)
                            ...
                            hyperlink_original = f'=HYPERLINK("{original_roi_path}", "View Original ROI Image {count}")'
    ...
    # Add 0.5 seconds delay between camera captures
    time.sleep(0.5)
